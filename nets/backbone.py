...
def autopad(k, p=None, d=1):  
    ...
    # Automatic padding of the input feature layer according to the Same principle
    if d > 1:
        # actual kernel-size
...
class Bottleneck(nn.Module):
    ...
    # C1 is the number of input channels and C2 is the number of output channels
...
class C2f(nn.Module):
    ...
    def forward(self, x):
        # A convolution is performed and then divided into two parts, each channel being C
        y = list(self.cv1(x).split((self.c, self.c), 1))
        # The residual structure is retained each time it is carried out and then stacked on top of each other to dense the residuals
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
    
class SPPF(nn.Module):
    # SPP structure, 5, 9, 13 maximum pooling nucleus maximum
...
class Backbone(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dark2(x)
        #-----------------------------------------------#
        #   dark3 has an output of 256, 80, 80, which is a valid feature layer
        #-----------------------------------------------#
        x = self.dark3(x)
        feat1 = x
        #-----------------------------------------------#
        #   dark4 has an output of 512, 40, 40, which is a valid feature layer
        #-----------------------------------------------#
        x = self.dark4(x)
        feat2 = x
        #-----------------------------------------------#
