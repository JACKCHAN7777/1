...
class DecodeBox():
    ...
    def decode_box(self, inputs):
        ...
        # Obtain the center width and height coordinates
    ...
    def yolo_correct_boxes(self, box_xy, box_wh, input_shape, image_shape, letterbox_image):
        #-----------------------------------------------------------------#
        #   The y-axis is placed in front of it to facilitate multiplication of the width and height of the prediction box and the image
        #-----------------------------------------------------------------#
        box_yx = box_xy[..., ::-1]
        box_hw = box_wh[..., ::-1]
        ...
        if letterbox_image:
            #-----------------------------------------------------------------#
            #   The offset is the offset of the effective area of the image relative to the upper left corner of the image
            # new_shape refers to the aspect and height scaling case
            #-----------------------------------------------------------------#
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
        #----------------------------------------------------------#
        #   Convert the format of the forecast to the format in the upper left corner and the lower right corner.
        ...
        for i, image_pred in enumerate(prediction):
            ...
            #   class_conf  [num_anchors, 1]    Kind confidence
            #   class_pred  [num_anchors, 1]    Types
            #----------------------------------------------------------#
            ...
            #----------------------------------------------------------#
            #   Prediction results are filtered based on confidence
            #----------------------------------------------------------#
            ...
            #------------------------------------------#
            #   Get all the kinds included in the predictions
            #------------------------------------------#
            ...
            for c in unique_labels:
                #------------------------------------------#
                #   Obtain all predictions filtered by a certain type of score
                #------------------------------------------#
                detections_class = detections[detections[:, -1] == c]
                #------------------------------------------#
                #   It's faster to use the official non-maximum suppression!
                # Filter out the boxes with the highest scores of the same type in a certain area
                #------------------------------------------#
                keep = nms(
                    detections_class[:, :4],
                    detections_class[:, 4],
                ...
                #  Sorted by confidence in the presence of objects
                # _, conf_sort_index = torch.sort(detections_class[:, 4]*detections_class[:, 5], descending=True)
                # detections_class = detections_class[conf_sort_index]
                # max_detections = []
                # while detections_class.size(0):
                #     # Take out this type with the highest confidence level, judge step by step, judge whether the degree of overlap is greater than nms_thres, and remove it if so
...
if __name__ == "__main__":
    ...
    #---------------------------------------------------#
    #   Modulates each feature layer of the predicted value to a true value
    #---------------------------------------------------#
    def get_anchors_and_decode(input, input_shape, anchors, anchors_mask, num_classes):
        ...
        #-----------------------------------------------#
        #   If the input is 640x640 input_shape = [640, 640]  input_height = 20, input_width = 20
        #   640 / 20 = 32
        ...
        #-------------------------------------------------#
        #   The size of the scaled_anchors obtained is relative to the feature layer
        ...
        prediction = input.view(batch_size, len(anchors_mask[2]),
                                num_classes + 5, input_height, input_width).permute(0, 1, 3, 4, 2).contipansus()
        ...
        #-----------------------------------------------#
        #   Width and height adjustment parameters for the prior box
        #-----------------------------------------------#
        w = torch.sigmoid(prediction[..., 2]) 
        h = torch.sigmoid(prediction[..., 3]) 
        #-----------------------------------------------#
        #  Gain confidence in whether there is an object 0 - 1
        #-----------------------------------------------#
        ...
        #----------------------------------------------------------#
        #   Generate grid, a priori box center, top left corner of the grid
        #   batch_size,3,20,20
        #   range(20)
        #   [
        #       [0, 1, 2, 3 ……, 19], 
        ...
        #----------------------------------------------------------#
        #   Use the prediction results to make adjustments to the prior box
        # First adjust the center of the prior box, offset from the center of the prior box to the lower right corner
        # Adjust the width and height of the prior box.
